...
# Set MLflow configuration from environment variables
...
class ModelTrainer:
    ...
    def train_model(self,X_train,y_train,x_test,y_test):
        ...
        preprocessor_path = self.data_transformation_artifact.transformed_object_file_path
        preprocessor = load_object(file_path=preprocessor_path)

        # Set MLflow to local tracking
        mlflow.set_tracking_uri("file:./mlruns")
        
        # Start a single MLflow run for the entire training process
        with mlflow.start_run():
            ...
            # Evaluate model and log metrics for both train and test sets
            ...
            # Log metrics with prefixes to distinguish them
            ...
            # Log the preprocessor object as an artifact
            mlflow.log_artifact(preprocessor_path, "preprocessor")
            logging.info(f"Logged preprocessor artifact from: {preprocessor_path}")
            ...
            logging.info(f"Logged model: {best_model_name}")

        model_dir_path = os.path.dirname(self.model_trainer_config.trained_model_file_path)
        os.makedirs(model_dir_path,exist_ok=True)

        Network_Model=NetworkModel(preprocessor=preprocessor,model=best_model)
        #model pusher - save only to final_models
